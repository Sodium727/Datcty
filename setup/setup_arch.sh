...
# Packages
sudo pacman -Sy --noconfirm --needed git base-devel clang wl-clipboard less qbittorrent gdb ripgrep neovim imv bat eza zoxide fcitx5 fcitx5-unikey fcitx5-config-qt mpv zsh kitty sddm firefox cliphist python-pynvim tmux git-zsh-completion libnotify flatpak ttf-jetbrains-mono-nerd hyprlock waybar hyprland hyprpaper hyprpolkitagent hyprsunset hyprpicker 7zip

# Install yay
git clone https://aur.archlinux.org/yay-bin.git && cd ~/yay-bin && makepkg -si

# other Hyprland packages from AUR
yay -S --noconfirm --needed hyprshot waypaper wlogout
